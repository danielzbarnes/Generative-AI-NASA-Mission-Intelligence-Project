...
class ChromaEmbeddingPipelineTextOnly:
    ...
    def __init__(self, 
        ...
        self.chroma_client = chromadb.PersistentClient(path=self.chroma_persist_directory, settings=Settings(anonymized_telemetry=False))
        ...
        """ 
        if self.collection_name in self.chroma_client.list_collections():
            self.collection = self.chroma_client.get_collection(name=self.collection_name)
            logger.info(f"Using existing collection: {self.collection_name}")
        else:
            self.collection = self.chroma_client.create_collection(name=self.collection_name, embedding_function=OpenAIEmbeddingFunction(model_name=self.embedding_model, api_key=self.openai_api_key))
    ...
    def check_document_exists(self, doc_id: str) -> bool:
        ...
        return self.collection.get(ids=[doc_id])['documents'] != []
    ...
    def get_embedding(self, text: str) -> List[float]:
        ...
        try:
            ...
            # embeddings = OpenAIEmbeddingFunction(model=self.embedding_model, api_key=self.openai_api_key)([text])
    ...
    def generate_document_id(self, file_path: Path, metadata: Dict[str, Any]) -> str:
        ...
        document_id = f"{metadata.get('mission', 'unknown')}_{metadata.get('source', 'unknown')}_chunk_{metadata.get('chunk_index', 0):04d}"
        return document_id
    ...
    def add_documents_to_collection(self, documents: List[Tuple[str, Dict[str, Any]]], 
        ...
        for i in range(0, len(documents), batch_size):
            ...
            for text, metadata in batch:
                ...
                # For new documents or replace mode, add to batch
                embedding = self.get_embedding(text)
                ids.append(doc_id)
                texts.append(text)
            ...
            if ids:
                try:
                    ...
                    stats['added'] += len(ids)
                    logger.debug(f"Added {len(ids)} documents from file: {file_path}")
                except Exception as e:
                    logger.error(f"Error adding documents from file {file_path}: {e}")
    ...
    def get_collection_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'document_count': self.collection.count() if hasattr(self.collection, "count") else "N/A",
            'metadata': self.collection.get()['metadatas'] if hasattr(self.collection, "get") else []
        } 
